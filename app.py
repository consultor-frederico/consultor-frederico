...
# COLOQUE SEU E-MAIL DO GOOGLE ABAIXO PARA RECEBER A PROPRIEDADE DOS ARQUIVOS
MEU_EMAIL_GOOGLE = "SEU_EMAIL@gmail.com" 
...
def main():
    ...
    if st.session_state.fase == 1:
        ...
        servico = st.selectbox("Tipo de Cálculo:", opcoes, index=0)
        relato = st.text_area("Resumo da Demanda:", value=d.get("relato", ""), height=100)

        if st.button("💬 Analisar Solicitação"):
            st.session_state.dados_form.update({"nome": nome, "tipo": tipo, "salario": salario, "relato": relato, "servico": servico})
    ...
    # --- FASE 2: CONFIRMAÇÃO ---
        ...
        if col_s.button("✅ Sim, prosseguir"): st.session_state.fase = 3; st.rerun()

    # --- FASE 3: TRAVA DE PERFIL ---
    ...
    # --- FASE 4: FINALIZAÇÃO ---
        ...
        if st.button("✅ Confirmar Agendamento"):
            with st.spinner("IA analisando complexidade..."):
                d = st.session_state.dados_form
                prompt = f"Analise: {d['relato']}. Aponte dificuldade (Baixa, Média, Alta) e valor sugerido (Mercado 2026)."
                ...
                st.success(f"Tudo pronto! Pasta: {link}")
